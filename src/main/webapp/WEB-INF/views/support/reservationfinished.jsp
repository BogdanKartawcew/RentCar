    ...
    <div class="panel panel-default">
        ...
        <div class="table-responsive">
            <table class="table table-hover">
                ...
                <tbody>
                <c:forEach items="${finished}" var="finished">
                    <tr>
                    <%--<c:if test="${reservation.carId == 1}">
                        <tr class="alert alert-danger">
                    </c:if>--%>
                    <c:if test="${finished.confirmed == 'false'}">
                        <tr class="alert alert-danger">
                    </c:if>
                    <td>${finished.reservationId}</td>
                    <td>${finished.clientId}</td>
                    <td>${finished.carId}</td>
                    <td>${finished.startDate}</td>
                    <td>${finished.startTime}</td>
                    <td>${finished.endDate}</td>
                    <td>${finished.endTime}</td>
                    ...
                    <c:if test="${finished.confirmed == 'false'}">
                        <td>
                            <a href="<c:url value='/support/reservation/editreservation-${finished.reservationId}' />"
                               class="btn btn-primary custom-width btn-sm">edit</a><%--primary--%>
                        </td>
                        <td>
                            <a href="<c:url value='/support/reservation/confirmreservation-${finished.reservationId}' />"
                               onclick="return confirm('Are you sure?')"
                               class="btn btn-success custom-width btn-sm">confirm</a>
                        </td>
                    </c:if>
                    <c:if test="${finished.confirmed == 'true'}">
                        <td>
                            <a href="<c:url value='/support/reservation/editreservation-${finished.reservationId}' />"
                    ...
                    <td>
                        <a href="<c:url value='/support/reservation/cancelreservation-${finished.reservationId}' />"
                           onclick="return confirm('Please confirm canceling.')"
                           class="btn btn-danger custom-width btn-sm">cancel</a>
                    </td>

                    <c:if test="${finished.active == 'true'}">
                        <td>
                            <a href="<c:url value='/support/reservation/endreservation-${finished.reservationId}' />"
                               onclick="return confirm('Please confirm ending.')"
                               class="btn btn-primary custom-width btn-sm">end</a>
                        </td>
                    </c:if>
                    <c:if test="${finished.active == 'false'}">
                        <td>
                            <a href="<c:url value='/support/reservation/endreservation-${finished.reservationId}' />"
                               onclick="return confirm('Please confirm ending.')"
